...
ResultsDF_Ctrl.to_csv('Human_and_Spombe_ResultsDF_No_AUG_dORF_water_included.csv')
...
for i in range(len(ResultsDF)):
    ...
    Human_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'SpomHuman_temp_seq.txt'), 'w')
    Human_temp_seq.write(HumanNoAUGdORF)
    Human_temp_seq.close()
    Homolog_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'SpomHomolog_temp_seq.txt'), 'w')
    Homolog_temp_seq.write(HomologNoAUGdORF)
    Homolog_temp_seq.close()

    #run water alignment with 2 temp 3UTR sequence files
    os.system("water SpomHuman_temp_seq.txt SpomHomolog_temp_seq.txt -gapopen 10.0 -gapextend 0.5 -outfile SpomTemp_Align.water >/dev/null 2>&1")

    #remove temporary sequence files - no longer needed - save space
    os.system("rm SpomHuman_temp_seq.txt")
    os.system("rm SpomHomolog_temp_seq.txt")

    #open water alignment file from path as string in python script
    with open('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis/SpomTemp_Align.water') as water_file:
    ...
    os.system("rm SpomTemp_Align.water")
    ...
    #add No AUG dORF water alignment results to temp df
        ...
        tempResultsDF.to_csv('Human_and_Spombe_ResultsDF_No_AUG_dORF_water_included.csv', header=None, mode='a')
