...
Homologs_rd = pd.read_csv("Spombe_RefSeq_dORF_containing_3UTRs_remove_duplicate_3UTRs.csv", index_col=0)
...
#open text file in path
    Homologs_dORF_temp_seqs = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'Spombe_Homologs_dORF_temp_seqs.txt'), 'w')
...
#loop through each entry in temp human and homolog dataframes
    for a in range(len(tempHuman_rd)):
        for b in range(len(tempHomologs_rd)):
            ...
            Human_ThreeUTR_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'Spombe_Human_ThreeUTR_temp_seq.txt'), 'w')
            Human_ThreeUTR_temp_seq.write(Human_temp3UTR)
            Human_ThreeUTR_temp_seq.close()
            Homologs_ThreeUTR_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'Spombe_Homologs_ThreeUTR_temp_seq.txt'), 'w')
            ...
            os.system("water Spombe_Human_ThreeUTR_temp_seq.txt Spombe_Homologs_ThreeUTR_temp_seq.txt -gapopen 10.0 -gapextend 0.5 -outfile Spombe_ThreeUTRTemp_Align.water >/dev/null 2>&1")

            #remove temporary sequence files - no longer needed - save space
            os.system("rm Spombe_Human_ThreeUTR_temp_seq.txt")
            os.system("rm Spombe_Homologs_ThreeUTR_temp_seq.txt")

            #open water alignment file from path as string in python script
            with open('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis/Spombe_ThreeUTRTemp_Align.water') as ThreeUTRwater_file:
            ...
            os.system("rm Spombe_ThreeUTRTemp_Align.water")
            ...
            for x in range(0, tempHuman_rd['Number of dORFs'][a]):
                ...
                Human_dORF_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'Spombe_Human_dORF_temp_seq.txt'), 'w')
                ...
                os.system("water Spombe_Human_dORF_temp_seq.txt Spombe_Homologs_dORF_temp_seqs.txt -gapopen 10.0 -gapextend 0.5 -outfile Spombe_dORFTemp_Align.water >/dev/null 2>&1")

                #remove temporary sequence files - no longer needed - save space
                os.system("rm Spombe_Human_dORF_temp_seq.txt")

                #open water alignment file from path as string in python script
                with open('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis/Spombe_dORFTemp_Align.water') as dORFwater_file:
                ...
                os.system("rm Spombe_dORFTemp_Align.water")
...
#remove temporary sequence files - no longer needed - save space
    os.system("rm Spombe_Homologs_dORF_temp_seqs.txt")

#save dataframe as csv in current directory
ResultsDF.to_csv('Human_and_Spombe_ResultsDF.csv')
