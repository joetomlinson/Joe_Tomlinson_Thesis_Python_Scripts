...
ResultsDF = pd.read_csv("Human_and_Xtropicalis_ResultsDF.csv", index_col=0)
...
ResultsDF_water_restriction.to_csv('Human_and_Xtropicalis_ResultsDF_water_length_restriction.csv')
...
for i in range(len(ResultsDF)):
    ...
    if tempResultsDF['3UTR Water Length'][0] >= MIN3UTR and tempResultsDF['dORF Water Length'][0] >= MINdORF:
        ...
        tempResultsDF.to_csv('Human_and_Xtropicalis_ResultsDF_water_length_restriction.csv', header=None, mode='a')
    else:
        continue
...
#load dataframes from .csv files and reset indexes
ResultsDF_no_dup = pd.read_csv("Human_and_Xtropicalis_ResultsDF_water_length_restriction.csv", index_col=0)
...
ResultsDF_no_dup.to_csv('Human_and_Xtropicalis_ResultsDF_water_length_restriction_and_human_dORF_ID_duplicates_removed.csv')
