...
ResultsDF_Ctrl.to_csv('Human_and_Mmusculus_ResultsDF_Ctrl_100_upstream_water_included.csv')
...
for i in range(len(ResultsDF)):
    ...
    if HumanCtrlstart > -1 and HomologCtrlstart > -1:
        ...
        Human_Ctrl_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'MmusHuman_Ctrl_temp_seq.txt'), 'w')
        Human_Ctrl_temp_seq.write(HumanCtrlSeq)
        Human_Ctrl_temp_seq.close()
        Homolog_Ctrl_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'MmusHomolog_Ctrl_temp_seq.txt'), 'w')
        Homolog_Ctrl_temp_seq.write(HomologCtrlSeq)
        Homolog_Ctrl_temp_seq.close()

        #run water alignment with 2 temp 3UTR sequence files
        os.system("water MmusHuman_Ctrl_temp_seq.txt MmusHomolog_Ctrl_temp_seq.txt -gapopen 10.0 -gapextend 0.5 -outfile MmusCtrlTemp_Align.water >/dev/null 2>&1")

        #remove temporary sequence files - no longer needed - save space
        os.system("rm MmusHuman_Ctrl_temp_seq.txt")
        os.system("rm MmusHomolog_Ctrl_temp_seq.txt")

        #open water alignment file from path as string in python script
        with open('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis/MmusCtrlTemp_Align.water') as Ctrlwater_file:
        ...
        os.system("rm MmusCtrlTemp_Align.water")
        ...
        if CtrlTemp_Length >= HumanCtrlLen or CtrlTemp_Length >= HomologCtrlLen:
            ...
            tempResultsDF.to_csv('Human_and_Mmusculus_ResultsDF_Ctrl_100_upstream_water_included.csv', header=None, mode='a')
        else:
            continue
    else:
        continue
