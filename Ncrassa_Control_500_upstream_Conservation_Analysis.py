...
ResultsDF = pd.read_csv("Human_and_Ncrassa_ResultsDF_water_length_restriction_and_human_dORF_ID_duplicates_removed.csv", index_col=0)
...
ResultsDF_Ctrl.to_csv('Human_and_Ncrassa_ResultsDF_Ctrl_500_upstream_water_included.csv')
...
for i in range(len(ResultsDF)):
    ...
    if HumanCtrlstart > -1 and HomologCtrlstart > -1:
        ...
        Human_Ctrl_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'NcraHuman_Ctrl_temp_seq.txt'), 'w')
        Human_Ctrl_temp_seq.write(HumanCtrlSeq)
        Human_Ctrl_temp_seq.close()
        Homolog_Ctrl_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'NcraHomolog_Ctrl_temp_seq.txt'), 'w')
        Homolog_Ctrl_temp_seq.write(HomologCtrlSeq)
        Homolog_Ctrl_temp_seq.close()

        #run water alignment with 2 temp 3UTR sequence files
        os.system("water NcraHuman_Ctrl_temp_seq.txt NcraHomolog_Ctrl_temp_seq.txt -gapopen 10.0 -gapextend 0.5 -outfile NcraCtrlTemp_Align.water >/dev/null 2>&1")

        #remove temporary sequence files - no longer needed - save space
        os.system("rm NcraHuman_Ctrl_temp_seq.txt")
        os.system("rm NcraHomolog_Ctrl_temp_seq.txt")

        #open water alignment file from path as string in python script
        with open('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis/NcraCtrlTemp_Align.water') as Ctrlwater_file:
        ...
        os.system("rm NcraCtrlTemp_Align.water")
        ...
        if CtrlTemp_Length >= HumanCtrlLen or CtrlTemp_Length >= HomologCtrlLen:
            ...
            tempResultsDF.to_csv('Human_and_Ncrassa_ResultsDF_Ctrl_500_upstream_water_included.csv', header=None, mode='a')
        else:
            continue
    else:
        continue
