#Python Script to analyse similarity of control sequences in human and homolog species of the dORF lengths finishing 500 bases upstream of the dORF start codon and add results to conservation analysis results
...
ResultsDF = pd.read_csv("Human_and_Dmelanogaster_ResultsDF_water_length_restriction_and_human_dORF_ID_duplicates_removed.csv", index_col=0)
...
for i in range(len(ResultsDF)):
    ...
    #Use human and homolog dORF locations to gather Ctrl start and end by shifting further into 3' UTR by 100 bases
    HumandORFstart = tempResultsDF['Human dORFstart'][0]
    HumandORFend = tempResultsDF['Human dORFend'][0]
    HumandORFlength = tempResultsDF['Human dORF Length'][0]
    HumanCtrlend = HumandORFstart - 500
    HumanCtrlstart = HumanCtrlend - HumandORFlength
    HomologdORFstart = tempResultsDF['Homolog dORFstart'][0]
    HomologdORFend = tempResultsDF['Homolog dORFend'][0]
    HomologdORFlength = tempResultsDF['Homolog dORF Length'][0]
    HomologCtrlend = HomologdORFstart - 500
    HomologCtrlstart = HomologCtrlend - HomologdORFlength
    #Only continue loop if the end of the Ctrl seqs falls within the 3' UTR
    if HumanCtrlstart > -1 and HomologCtrlstart > -1:
        #Gather Ctrl sequences using locations as index for 3' UTR string and calculate the length and get start and end by adding 1 to the indexes
        ...
        HomologCtrlSeq = tempResultsDF['Homolog 3UTRseq'][0][HomologCtrlstart:HomologCtrlend]
        HomologCtrlstart = HomologCtrlstart + 1
        HomologCtrlend = HomologCtrlend
        HomologCtrlLen = len(HomologCtrlSeq)
        if HumanCtrlLen == 0 or HomologCtrlLen ==0:
            continue
        #Write out Ctrl humand and homolog sequences into text files to be used with water tool in command line
        Human_Ctrl_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'DmelHuman_Ctrl_temp_seq.txt'), 'w')
        Human_Ctrl_temp_seq.write(HumanCtrlSeq)
        Human_Ctrl_temp_seq.close()
        Homolog_Ctrl_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'DmelHomolog_Ctrl_temp_seq.txt'), 'w')
        Homolog_Ctrl_temp_seq.write(HomologCtrlSeq)
        Homolog_Ctrl_temp_seq.close()

        #run water alignment with 2 temp 3UTR sequence files
        os.system("water DmelHuman_Ctrl_temp_seq.txt DmelHomolog_Ctrl_temp_seq.txt -gapopen 10.0 -gapextend 0.5 -outfile DmelCtrlTemp_Align.water >/dev/null 2>&1")

        #remove temporary sequence files - no longer needed - save space
        os.system("rm DmelHuman_Ctrl_temp_seq.txt")
        os.system("rm DmelHomolog_Ctrl_temp_seq.txt")

        #open water alignment file from path as string in python script
        with open('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis/DmelCtrlTemp_Align.water') as Ctrlwater_file:
        ...
        os.system("rm DmelCtrlTemp_Align.water")
        
        #if water alignment length is more or equal to the human or homolog Ctrl length then add results to dataframe if not the whole of either sequence is not aligned
        if CtrlTemp_Length >= HumanCtrlLen or CtrlTemp_Length >= HomologCtrlLen:
            #add details about Ctrl sequences to temp results dataframe
            ...
            tempResultsDF['Homolog CtrlStart'][0] = HomologCtrlstart
            tempResultsDF['Homolog CtrlEnd'][0] = HomologCtrlend
            tempResultsDF['Homolog Ctrl Length'][0] = HomologCtrlLen
            tempResultsDF['Homolog Ctrl Sequence'][0] = HomologCtrlSeq
            ...
            #Compare water alignments results for Ctrl compared to the 3' UTR between human and homolog - look at difference between dORF and 3 UTR +ive suggests dORF has greater % when comparing
            ...
            dORFCtrlDiff = dORFsimilarity - CtrlTemp_Similarity
            dORFCtrlDiff = format(dORFCtrlDiff, '.2f')
            tempResultsDF['dORF vs Ctrl Water Similarity Difference'] = [dORFCtrlDiff]
            
            #write results of temporary results dataframe into created.csv results file
            tempResultsDF.to_csv('Human_and_Dmelanogaster_ResultsDF_Ctrl_500_upstream_water_included.csv', header=None, mode='a')
        else:
            continue
    else:
        continue
