...
ResultsDF = pd.read_csv("Human_and_Athaliana_ResultsDF_duplicate_3UTR_comparisons_removed.csv", index_col=0)
...
ResultsDF_Ctrl.to_csv('Human_and_Athaliana_ResultsDF_Ctrl_middle_water_included.csv')
...
for i in range(len(ResultsDF)):
    ...
    if HumanCtrlend < tempResultsDF['Human 3UTR Length'][0]:
        ...
        if CtrlTemp_Length >= HumanCtrlLen:
            ...
            tempResultsDF.to_csv('Human_and_Athaliana_ResultsDF_Ctrl_middle_water_included.csv', header=None, mode='a')
        else:
            continue
    else:
        continue
