...
ResultsDF_Ctrl.to_csv('Human_and_Drerio_ResultsDF_No_AUG_dORF_water_included.csv')
...
for i in range(len(ResultsDF)):
    ...
    Human_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'DrerHuman_temp_seq.txt'), 'w')
    Human_temp_seq.write(HumanNoAUGdORF)
    Human_temp_seq.close()
    Homolog_temp_seq = open(os.path.join('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis', 'DrerHomolog_temp_seq.txt'), 'w')
    Homolog_temp_seq.write(HomologNoAUGdORF)
    Homolog_temp_seq.close()

    #run water alignment with 2 temp 3UTR sequence files
    os.system("water DrerHuman_temp_seq.txt DrerHomolog_temp_seq.txt -gapopen 10.0 -gapextend 0.5 -outfile DrerTemp_Align.water >/dev/null 2>&1")

    #remove temporary sequence files - no longer needed - save space
    os.system("rm DrerHuman_temp_seq.txt")
    os.system("rm DrerHomolog_temp_seq.txt")

    #open water alignment file from path as string in python script
    with open('/users/payjt2/PhD/Homolog_3UTR_and_dORF_comparison_analysis/Emboss_Water_Analysis/DrerTemp_Align.water') as water_file:
    ...
    os.system("rm DrerTemp_Align.water")
    ...
    #add No AUG dORF water alignment results to temp df
        ...
        tempResultsDF.to_csv('Human_and_Drerio_ResultsDF_No_AUG_dORF_water_included.csv', header=None, mode='a')
